...
export default class Reconciler {
  ...
  // hedges is held for upcoming re-enqueue support; today the loop only
  // queries via prisma directly because the queries are one-off.
  ...
  public start(): void {
    ...
    this.log.info(
      { intervalMs: ENV.HEDGER_RECONCILE_INTERVAL_MS },
      "reconciler started",
    );
  }
  ...
  private async recheck_one(market_id: string, prior_outcome: Outcome | null): Promise<void> {
    ...
    if (prior_outcome && live_outcome !== prior_outcome) {
      await this.handle_outcome_flip(market_id, market.polyMarketId, prior_outcome, live_outcome);
    }
  }
