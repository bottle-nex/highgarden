...
export default class Resolver {
  ...
  // hedges is unused today but kept for symmetry with the reconciler and
  // because the resolver may need to consult fills/exposure once we
  // surface "skipped due to unhedged residual" diagnostics.
  ...
  private async redeem_polygon_for_resolved(): Promise<void> {
    ...
    const ready = await this.markets.resolver_list_awaiting_redemption();
    for (const row of ready) {
      try {
  ...
  private async handle_redeem_outcome(
    ...
    if (outcome.kind === "submitted") {
      await this.markets.resolver_record_redeemed(market_id, outcome.txHash, new Date());
      this.log.info(
        {
          marketId: market_id,
    ...
    if (outcome.kind === "skipped_neg_risk" || outcome.kind === "skipped_no_condition_id") {
      await this.markets.resolver_append_note(market_id, `redeem_skipped: ${outcome.kind}`);
      this.log.warn(
        {
          marketId: market_id,
          polyMarketId: polymarket_market_id,
