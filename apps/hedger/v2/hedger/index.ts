...
export default class Hedger {
  ...
  constructor(
    ...
    this.processor = new HedgeProcessor(poly, hedges, markets, users);
  ...
  private async mark_hedge_failed(job_id: string, reason: string): Promise<void> {
    try {
      const hedge = await this.hedges.hedge_find_by_bull_job_id(job_id);
      if (!hedge) return;
      if (hedge.status === "FAILED") return;
      await this.hedges.hedge_mark_failed(hedge.id, reason);
    } catch (err) {
  ...
  private async maybe_pause_market(job_id: string, reason: string): Promise<void> {
    ...
    try {
      ...
      await this.hedges.exposure_set_paused(ctx.marketId, true);
      this.log.warn(
        { jobId: job_id, marketId: ctx.marketId, marketPda: ctx.solanaMarketPda, txSig: sig, reason },
  ...
  private async recover_stuck_hedging(): Promise<void> {
    ...
    this.log.warn(
      { count: stuck.length },
      "found hedges stuck in HEDGING from a prior crash",
    );
    for (const row of stuck) {
      await prisma.hedge.update({ where: { id: row.id }, data: { status: "PENDING" } });
      this.log.warn(
        {
          hedgeId: row.id,
          fillId: row.fillId,
