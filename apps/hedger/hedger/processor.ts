...
export default class HedgeProcessor {
    ...
    private async lookup_user(custodial_pubkey: string): Promise<CustodialUser> {
        ...
        if (!user) {
            throw new RetryableError(
                `no User row with custodialPublicKey=${custodial_pubkey}`,
            );
        }
        return user;
    }
    ...
    private async finalize_filled(
        ctx: ResolvedContext,
        result: { polymarketOrderId: string | null; filledShares: number; avgPriceCents: number | null },
    ): Promise<HedgeJobResult> {
    ...
    private async finalize_partial(
        ctx: ResolvedContext,
        initial: { polymarketOrderId: string | null; filledShares: number; avgPriceCents: number | null },
        target_price: number,
    ): Promise<HedgeJobResult> {
