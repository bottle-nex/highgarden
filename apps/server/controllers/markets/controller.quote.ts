 ...
 * least a week with no rollback. Sunset target: ~2 weeks after
 * SERVER_TRADE_ENDPOINT_ENABLED + NEXT_PUBLIC_USE_HEDGE_FIRST_TRADE
 * flip in prod.
 */
export default class QuoteController {
    ...
    static async process(req: Request, res: Response) {
        QuoteController.warn_deprecated_once();
        QuoteController.set_deprecation_headers(res);
    ...
    private static warn_deprecated_once(): void {
        if (QuoteController.deprecation_warned) return;
        QuoteController.deprecation_warned = true;
        console.warn(
            "[deprecated] POST /markets/:id/quote — use POST /markets/:id/trade" +
