...
export default function LandingInteractiveSection(): JSX.Element {
    ...
    function scrollToSection(i: number) {
        ...
        const target =
            sectionTop + (i / sections.length) * total + 1;
    ...
    return (
        <section
            ref={sectionRef}
            className="w-full relative h-[500vh] bg-black text-white"
        >
        ...
        <div className="flex flex-col gap-y-4">
            ...
            <ul className="flex flex-col gap-y-2">
                {steps.map((s) => (
                    <li
                        key={s.label}
                        className="flex items-center gap-x-3 text-sm text-white/80"
                    >
