        ...
        <section className="w-full bg-dark-alpha py-20">
            <section className="max-w-7xl mx-auto w-full grid grid-cols-2">
                <div className="col-span-1">
                    <h1 className="text-5xl">One prediction market. Every edge Solana gives you.</h1>
                </div>
                <div className="col-span-1">
                    <p className="text-2xl">
                        SolMarket fuses Polymarket-grade liquidity with sub-second Solana execution — so you can trade real-world outcomes without bridging, waiting, or paying Polygon gas.
                    </p>
                </div>
            </section>
            <div className="grid w-full grid-cols-1 gap-0 md:grid-cols-3 mt-20">
                {CARDS.map((card) => {
                    ...
                    return (
                        ...
                        >
                            {isActive ? <ActiveCard card={card} /> : <InactiveCard label={card.label} />}
                        </div>
                    );
                })}
            </div>
...
function InactiveCard({ label }: { label: string }): JSX.Element {
    ...
    return (
        <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="flex h-full w-full flex-col p-20">
        ...
        <div className="flex h-full w-full flex-col p-20 relative">
            <div className="flex items-center justify-between text-xs">
                ...
                <span className="flex items-center gap-1">
                    ...
                    <span className={`bg-black px-1 py-0.5 ${card.accentText}`}>{card.context}</span>
                </span>
            </div>
            ...
            <div className="mt-3 flex flex-1 flex-col justify-end gap-2 bg-black p-4 relative">
                {card.questions.map((q) => (
                    <div
                        key={q}
                        className={cn("self-end border border-white/15 bg-neutral-950 px-3 py-2 text-xs text-neutral-200 cursor-pointer", card.questionHover)}
                    >
                        {q}
                    </div>
                ))}
...
export function EdgeArrows({ borderColor = "border-black" }: { borderColor?: string }) {
    return (
        <>
            <span className={`aspect-square w-4 h-4 absolute -bottom-5 -right-5 border ${borderColor} border-r-0 border-b-0`} />
            <span className={`aspect-square w-4 h-4 absolute -top-5 -right-5 border ${borderColor} border-r-0 border-t-0`} />
            <span className={`aspect-square w-4 h-4 absolute -bottom-5 -left-5 border ${borderColor} border-l-0 border-b-0`} />
            <span className={`aspect-square w-4 h-4 absolute -top-5 -left-5 border ${borderColor} border-l-0 border-t-0`} />
        </>
    )
}