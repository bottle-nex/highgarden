...
class HighgardenInitializer {
    ...
    private read_config(): Config {
        ...
        const home = homedir();
        const admin_path = process.env.HIGHGARDEN_ADMIN_KEYPAIR
            ?? resolve(home, ".config/solana/solmarket-admin.json");
        const oracle_path = process.env.HIGHGARDEN_ORACLE_KEYPAIR
            ?? resolve(home, ".config/solana/solmarket-oracle.json");
        const quote_path = process.env.HIGHGARDEN_QUOTE_KEYPAIR
            ?? resolve(home, ".config/solana/solmarket-quote.json");
