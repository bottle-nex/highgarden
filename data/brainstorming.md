...
Build a **Polymarket-style prediction market on Solana**, but solve the *cold-start liquidity problem* by **mirroring Polymarket's order book** onto our platform and **hedging trades back to Polymarket** (with a spread for profit + risk buffer).

---

## Background: Why this problem exists

- **Polymarket** is the dominant prediction market, but it lives on **Polygon**. Its contracts are *mildly centralised, mildly decentralised* — a hybrid design.
...
- The **hard part is liquidity**. A prediction market with no traders = no market.
  - **Trepa** (Solana prediction market) tried this and failed — *market makers never showed up*.
...
Instead of *waiting* for market makers, **become the market maker ourselves** by proxying Polymarket's liquidity onto Solana.
...
- A trade executes on Solana → we then have to *separately* execute the hedge on Polygon.
...
- **ToS language** on automated trading / geographic restrictions — Polymarket blocks US users; depending on where *you* operate from, this matters.
...
- Polymarket-quality UX is the *reason users come to us instead of bridging*, so this matters.
...
- **Market coverage:** Mirror *all* Polymarket markets, or only the top-N most liquid ones (where our hedge is most reliable)?
